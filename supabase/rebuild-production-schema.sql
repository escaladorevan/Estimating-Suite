...
create policy app_profiles_select on public.app_user_profiles
  for select to authenticated using (id = auth.uid() or app_private.current_app_role() = 'admin');
create policy app_profiles_admin_write on public.app_user_profiles
  for all to authenticated using (app_private.current_app_role() = 'admin') with check (app_private.current_app_role() = 'admin');
...
create policy estimates_write on public.estimates
  ...
  using (app_private.can_write_estimate(id))
  with check (app_private.can_write_estimate_header(document_type));

create policy estimate_areas_select on public.estimate_areas
  for select to authenticated using (true);
create policy estimate_areas_write on public.estimate_areas
  for all to authenticated
  using (app_private.can_write_estimate(estimate_id))
  with check (app_private.can_write_estimate(estimate_id));
...
create policy estimate_sections_write on public.estimate_sections
  for all to authenticated
  using (app_private.can_write_estimate_area(area_id))
  with check (app_private.can_write_estimate_area(area_id));

create policy estimate_items_select on public.estimate_items
  for select to authenticated using (true);
create policy estimate_items_write on public.estimate_items
  for all to authenticated
  using (app_private.can_write_estimate_section(section_id))
  with check (app_private.can_write_estimate_section(section_id));
...
create policy change_orders_write on public.change_orders
  for all to authenticated
  using (app_private.can_write_estimating() or app_private.can_write_pm())
  with check (
    (app_private.can_write_estimating() or app_private.can_write_pm())
    and (estimate_id is null or app_private.can_write_estimate(estimate_id))
  );

create policy purchase_orders_select on public.purchase_orders
  for select to authenticated using (true);
create policy purchase_orders_write on public.purchase_orders
  for all to authenticated using (app_private.can_write_pm()) with check (app_private.can_write_pm());
...
create policy activity_events_insert on public.activity_events
  for insert to authenticated with check (app_private.can_write_shared());
...
create policy historical_results_write on public.historical_results
  for all to authenticated using (app_private.current_app_role() in ('admin', 'estimator')) with check (app_private.current_app_role() in ('admin', 'estimator'));
