...
type SupabasePMNoteClient = {
  from: (table: "pm_notes") => any;
};
...
export async function savePMNote(note: PMNote, client: SupabasePMNoteClient | null = supabase): Promise<PMNote> {
  if (!client) return note;
  if (isUuid(note.id)) {
    const { data, error } = await client.from("pm_notes").update(mapPMNoteToWrite(note)).eq("id", note.id).select("*").single();
    if (error) throw error;
    return data ? mapPMNoteFromRow(data) : note;
  } else {
    const { data, error } = await client.from("pm_notes").insert(mapPMNoteToInsert(note)).select("*").single();
    if (error) throw error;
    return data ? mapPMNoteFromRow(data) : note;
  }
}

export async function deletePMNote(noteId: string, client: SupabasePMNoteClient | null = supabase): Promise<void> {
  if (!client) return;
